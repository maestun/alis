...
static void cleave(void) {
    ...
    if (alis.fseq == 0)
    {
        ...
        if (vacc_offset != 0)
        {
            ...
            alis.script->context->_0x08_script_ret_offset = vram_read32(vacc_offset);
    ...
    else
    {
        ...
        {
            alis.script->context->_0x0c_vacc_offset = 0;
            alis.script->pc = vram_read32(alis.script->vacc_off);
            return;
        }
    }
    ...
    s32 new_pc = vram_read32(alis.script->vacc_off);
...
//            printf(" [finent %.6x] \n", alis.finent);
    ...
    // TODO: present1 is stored to 27e20 but it should be in 278e0
...
static void cload(void) {
//    printf("\nCLOAD\nA3 %.6x  A6 %.6x\n", alis.script->pc, alis.script->vram_org);
...
static void csend(void) {
    ...
    while (1)
    {
        ...
        *(s16 *)(alis.mem + script->vram_org + old_vacc) = alis.varD7;
...
static void ctstform(void) {
    debug(EDebugWarning, " /* CHECK */");
    alis.wcx = vram_read16(0);
    alis.wcy = vram_read16(2);
    alis.wcz = vram_read16(4);
    readexec_opername();
    alis.matmask = alis.varD7;
    readexec_opername();
...
static void cscheart(void) {
    ...
    if (screen_id != 0)
    {
        ...
        scene->unknown0x0a = alis.varD7;
        scene->unknown0x0c = alis.varD6;
        scene->state |= 0x80;
    }
}
...
static void cscpos(void) {
    ...
    {
        SceneVariables *scene = SCENE_VAR(screen_id);
        scene->newx = alis.varD7;
        scene->newy = alis.varD6;
        scene->state |= 0x80;
    }
}
...
static void cscsize(void) {
    ...
    {
        SceneVariables *scene = SCENE_VAR(screen_id);
        scene->width = alis.varD7;
        scene->height = alis.varD6;
        scene->state |= 0x80;
    }
}
...
static void cschoriz(void) {
    ...
    if (screen_id != '\0')
    {
        SceneVariables *scene = SCENE_VAR(screen_id);
        readexec_opername();
        scene->unknown0x20 = (u8)alis.varD7;
        readexec_opername();
        scene->unknown0x21 = (u8)alis.varD7;
        readexec_opername();
        scene->unknown0x22 = (u8)alis.varD7;
        scene->state |= 0x80;
    }
}
...
static void cscvertic(void) {
    ...
    if (screen_id != '\0')
    {
        SceneVariables *scene = SCENE_VAR(screen_id);
        readexec_opername();
        scene->unknown0x23 = (u8)alis.varD7;
        readexec_opername();
        scene->unknown0x24 = (u8)alis.varD7;
        readexec_opername();
        scene->unknown0x25 = (u8)alis.varD7;
        scene->state |= 0x80;
    }
}
...
static void cshrink(void) {
    ...
    if (*(u16 *)(data + 4) != 0)
    {
        ...
        *(u16 *)(data + 4) = 0;
        ...
        debug(EDebugVerbose, " (NAME: %s, ID: 0x%x) ", alis.script->name, alis.script->header.id);
...
static void cscfollow(void) {
    ...
    readexec_opername();
    *(s16 *)(screen + 0x60) = alis.varD7;
    readexec_opername();
    *(s8 *)(screen + 0x84) = (s8)alis.varD7;
    readexec_opername();
    *(s16 *)(screen + 0x86) = alis.varD7;
    readexec_opername();
    *(s16 *)(screen + 0x88) = alis.varD7;
    readexec_opername();
    *(s16 *)(screen + 0x8a) = alis.varD7;
    readexec_opername();
    *(s16 *)(screen + 0x8c) = alis.varD7;
    readexec_opername();
    *(s16 *)(screen + 0x8e) = alis.varD7;
    readexec_opername();
    *(s16 *)(screen + 0x90) = alis.varD7;
}
...
static void cstart(s32 offset) {
    if (alis.fseq == '\0')
    {
        if (alis.script->context->_0x0c_vacc_offset == 0)
        {
            ...
            vram_write32(vacc_offset, alis.script->context->_0x08_script_ret_offset);
    ...
    else
    {
        if (alis.script->context->_0x0c_vacc_offset == 0)
        {
            ...
            vram_write32(alis.script->vacc_off, alis.script->pc);
        }
    ...
    if (script_id != 0)
    {
        ...
        while ((entidx = *(u16 *)(alis.mem + alis.atent + 4 + entidx)))
        {
            ...
            else if (start <= script->context->_0x14_script_org_offset)
            {
                ...
                {
                    script->vacc_off -= 4;
                    *(s32 *)(alis.mem + script->vram_org + script->vacc_off) -= length;
                }
