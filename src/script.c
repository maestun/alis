...
sAlisScript * script_init(char * name, u8 * data, u32 data_sz) {
    ...
    script->header.b_0x17 = *(u8 *)(data + 2);
    script->header.b_0x00 = *(u8 *)(data + 3);
    script->header.code_start_offset_minus_2 = swap16(*(u16 *)(data + 4), vm.platform);
    script->header.offset_to_subscript_routine = swap32(*(u32 *)(data + 6), vm.platform);
    script->header.offset_to_interrupt_routine = swap32(*(u32 *)(data + 10), vm.platform);
...
//    sScriptDebug debug_data = script_debug_data[script->header.id];
    ...
    script->context._0x8_script_ret_offset = script->data_org + script->header.code_start_offset_minus_2 + 2;
    script->context._0x2e_script_header_byte_2 = script->header.b_0x17;
...
void script_debug(sAlisScript * script) {
    ...
    u8 header_len = script->header.code_start_offset_minus_2 + sizeof(u16) /* script ID length */;
